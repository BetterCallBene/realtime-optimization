function varargout = QuadAnim41(varargin)
...
% QuadAnim41 MATLAB code for QuadAnim41.fig
%      QuadAnim41, by itself, creates a new QuadAnim41 or raises the existing
%      singleton*.
%
%      H = QuadAnim41 returns the handle to a new QuadAnim41 or the handle to
%      the existing singleton*.
%
%      QuadAnim41('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QuadAnim41.M with the given input arguments.
%
%      QuadAnim41('Property','Value',...) creates a new QuadAnim41 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before QuadAnim41_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to QuadAnim41_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help QuadAnim41

% Last Modified by GUIDE v2.5 30-Apr-2014 10:38:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...cxlc
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @QuadAnim41_OpeningFcn, ...
                   'gui_OutputFcn',  @QuadAnim41_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before QuadAnim41 is made visible.
function QuadAnim41_OpeningFcn(hObject, eventdata, handles, varargin)
global visualization
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to QuadAnim41 (see VARARGIN)

% Choose default command line output for QuadAnim41
...
% so window can get raised using QuadAnim41.

%Get current data
A = visualization.yout;
...
% TODO: Visualisation of a X Model
...
% UIWAIT makes QuadAnim41 wait for user response (see UIRESUME)
% uiwait(handles.figure1);



% --- Outputs from this function are returned to the command line.
function varargout = QuadAnim41_OutputFcn(hObject, eventdata, handles)
...
%%%%% Plot our stuff
        axes(handles.axes5);
        hold off
        X = A(j,13); Y = A(j,14); Z = A(j,15);
        
        quiver3(0,0,0 , X,Y,Z, 'LineWidth',3);
        view(-30, 45);
        wind_max = max(max(A(:,13:15))) + 0.1;
        wind_min = min(min(A(:,13:15))) - 0.1;
        
        wind_lim = [wind_min, wind_max];
