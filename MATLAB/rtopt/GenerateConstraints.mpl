...
eq := Vector([VectorCalculus:-`+`(VectorCalculus:-`+`(VectorCalculus:-`+`(VectorCalculus:-`+`(q[1]^2, q[2]^2), q[3]^2), q[4]^2), -1)]);
...
if TEST = false then Matlab(ConstraintsCount, resultname = "CountConstraints", defaulttype = integer, output = tmpRTConstraintsCount); Matlab(CountJacobi, resultname = "CountJacobi", defaulttype = integer, output = tmpRTConstraintsJacobiCount); Matlab(eval(([codegen:-optimize])(eqMatrix, tryhard)), defaulttype = integer, output = tmpRTConstraintsFunction); VectorCalculus:-`*`(Matlab(eval(([codegen:-optimize])(J, tryhard)), defaulttype = integer, output = tmpRTConstraintsJacobi), Matlab(eval(([codegen:-optimize])(H, tryhard)), defaulttype = integer, output = tmpRTConstraintsHesse)) else Matlab(eval(([codegen:-optimize])(eqMatrix, tryhard)), defaulttype = integer) end if;
