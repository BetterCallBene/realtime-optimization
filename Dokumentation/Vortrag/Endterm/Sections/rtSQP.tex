...
%Draw actual time
\draw<1-2>[dashed] (-.65,3) -- (-.65,-1) node[anchor=north] {$t_{act}$};
\draw<3->[dashed] (0,3) -- (0,-1) node[anchor=north] {$t_{act}$};
...
\end{enumerate}
\onslide<5->
$ \Rightarrow $ with horizon $ 18 $ this is $ 28\% $ faster than backslash-operator
\end{frame}
...
\draw[ thick, ->] (-0.5,1) -- (8,1) node[anchor=west] {time};
	...
	\draw[thick, decorate] (0,1.3) -- (6,1.3) node[midway, above,yshift=6pt,]{Horizon at t};
	\draw[thick, decorate] (7,0.4) -- (1,0.4) node[midway, below,yshift=-6pt,]{Horizon at t+1};
	\end{tikzpicture}
\end{figure}
\begin{block}{ }
$ \Rightarrow $ compromise between accuracy and speed
\end{block}
\end{frame}
