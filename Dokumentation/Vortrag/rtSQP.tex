...
\foreach  \t in  {-2, -1, +1}
	\draw[thick]  (\t cm, 1pt)  -- (\t cm, -1pt) node[anchor=north] {$t \t $ };
\draw[thick]  (0cm , 1pt)  -- (0 cm, -1pt) node[anchor=north] {$t$ };
...
\item<3-> {\color{green} calculate $y$ (Riccati Part II)}
\item<4-> {\color{blue} prepare $u_t$ (Newton \& Riccati Part I)} 
\end{enumerate}
\end{frame}
